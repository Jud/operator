...
/// Errors that can occur when executing JXA scripts via osascript.
...
public enum OsascriptRunner {
    /// Execute a JXA script without blocking the cooperative thread pool.
    ...
    /// Remove a temp file created by ``writeTempFile(_:)``.
    ///
    ...
    /// Decode JXA JSON output into `[DiscoveredSession]`.

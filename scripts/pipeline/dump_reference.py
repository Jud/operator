...
def dump_reference(model_id, output_dir, prompt_cache_dir=None):
    ...
    for i, layer in enumerate(model.model.layers):
        ...
        if layer_type == "linear_attention":
            ...
            hooks.append(layer.linear_attn.in_proj_qkv.register_forward_hook(make_hook(f"{prefix}_deltanet_qkv")))
            hooks.append(layer.linear_attn.in_proj_z.register_forward_hook(make_hook(f"{prefix}_deltanet_z")))
            hooks.append(layer.linear_attn.out_proj.register_forward_hook(make_hook(f"{prefix}_deltanet_out")))
            delta_idx += 1
        else:
            # Standard attention

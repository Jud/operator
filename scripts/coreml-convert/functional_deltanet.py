...
def patch_transformers():
    ...
    def fixed_size_cache_update(self, key_states, value_states, layer_idx, cache_kwargs=None):
        if (self.key_cache[layer_idx] is not None
            ...
            pos = cache_position.long()
            idx = pos.view(1, 1, 1, 1).expand_as(key_states)
            new_k = self.key_cache[layer_idx].scatter(2, idx, key_states)
            new_v = self.value_cache[layer_idx].scatter(2, idx, value_states)

"""Convert Qwen3.5 monolith decode model with Hadamard-rotated + 2-bit palettized lm_head.

Copies the monolith KV conversion pipeline but:
1. Applies Hadamard rotation to lm_head weights (offline, during conversion)
2. Inserts a Hadamard rotation matmul before lm_head (runtime, per token)
3. After CoreML conversion, palettizes only the lm_head to 2-bit

Produces three models for comparison:
  --mode baseline  → FP16 monolith (same as convert_monolith_kv.py)
  --mode norot     → 2-bit palettized lm_head, no rotation (control)
  --mode hadamard  → Hadamard rotation + 2-bit palettized lm_head

Usage:
    /tmp/coreml-venv/bin/python scripts/experiments/quip-hadamard-lmhead/convert_hadamard.py \
        Qwen/Qwen3.5-0.8B --mode hadamard --output models/monolith_hadamard_2bit/

    # Control (no rotation):
    /tmp/coreml-venv/bin/python scripts/experiments/quip-hadamard-lmhead/convert_hadamard.py \
        Qwen/Qwen3.5-0.8B --mode norot --output models/monolith_norot_2bit/
"""
...
class MonolithDecodeKV(nn.Module):
    ...
    def __init__(self, hf_model, config, hadamard_rotate=False):
        ...
        if hadamard_rotate:
            H = hadamard_matrix(tc.hidden_size)
            self.register_buffer("H_T", H.T.contiguous())
...
def find_lm_head_weight_name(mlmodel):
    """Find the lm_head weight name in the CoreML model."""
    metadata = cto.coreml.get_weights_metadata(mlmodel, weight_threshold=2048)
    candidates = []
    for name, meta in metadata.items():
        shape = meta.val.shape if hasattr(meta, 'val') else None
        candidates.append((name, shape))
        print(f"  Weight: {name} shape={shape}")

    # The lm_head weight should be the largest one (248320 × 1024)
    for name, shape in candidates:
        if shape is not None and len(shape) == 2:
            if shape[0] > 100000:  # vocab size
                return name

    # Fallback: look for 'lm_head' in name
    for name, shape in candidates:
        if 'lm_head' in name or 'head' in name:
            return name

    raise RuntimeError(f"Could not find lm_head weight. Candidates: {candidates}")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("model_id")
    parser.add_argument("--output", default="models/monolith_hadamard_2bit/")
    parser.add_argument("--max-seq-len", type=int, default=256)
    parser.add_argument("--mode", choices=["baseline", "norot", "hadamard"], default="hadamard")
    ...
    use_hadamard = args.mode == "hadamard"
    use_palettize = args.mode in ("hadamard", "norot")

    print(f"Loading {args.model_id}...")
    print(f"  Mode: {args.mode}, nbits: {args.nbits}")
    ...
    # Rotate lm_head weights if using Hadamard
    if use_hadamard:
        print(f"\nApplying Hadamard rotation to lm_head weights [{model.lm_head.weight.shape}]...")
        H = hadamard_matrix(hidden_size)
        with torch.no_grad():
            W_orig_std = model.lm_head.weight.std(dim=0)
            model.lm_head.weight.copy_(model.lm_head.weight @ H.T)
            W_rot_std = model.lm_head.weight.std(dim=0)
            print(f"  Per-coord std range: [{W_orig_std.min():.4f}, {W_orig_std.max():.4f}] → "
                  f"[{W_rot_std.min():.4f}, {W_rot_std.max():.4f}]")
    ...
    wrapper = MonolithDecodeKV(model, config, hadamard_rotate=use_hadamard)
    wrapper.eval()
    ...
    # Palettize lm_head if requested
    if use_palettize:
        print(f"\nPalettizing lm_head to {args.nbits}-bit...")
        print("Finding lm_head weight name...")
        lm_head_name = find_lm_head_weight_name(mlmodel)
        ...
        print(f"  Palettization complete")
    ...
    # Export test inputs (same as convert_monolith_kv.py)
    print("Exporting test inputs...")
    test_dir = os.path.join(args.output, "test_inputs")
    os.makedirs(test_dir, exist_ok=True)

    # Reload original model for prefill (rotation was applied to the conversion model,
    # but prefill should use the unmodified model to generate reference states)
    print("  Loading fresh model for prefill reference...")
    ref_model = AutoModelForCausalLM.from_pretrained(args.model_id, dtype=torch.float32, config=config)
    ref_model.eval()
    ...
    with torch.no_grad():
        out = ref_model(input_ids, use_cache=True)
    ...
    # Reference token from unmodified model
    ...
    with torch.no_grad():
        ...
        hidden = ref_model.model.embed_tokens(torch.tensor([[first_token]]))
        ref_logits = manual_decode_step(ref_model, tc.layer_types, hidden,
